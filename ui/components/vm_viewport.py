...
Embedding flow:
 1. Poll for VM window using EnumWindows
 2. Confirm window has a non-zero client area (rendered at least once)
 3. Use QWindow.fromWinId(hwnd) and QWidget.createWindowContainer(window)
 4. Strip borders using SetWindowLong
...
def find_vm_hwnd(vm_name: str) -> int:
    """
    Find the VirtualBox VM window by title (case-insensitive).
    ...
    Accepts any visible window whose title contains vm_name AND 'virtualbox',
    excluding the Manager window.
    """
    found_hwnd = []
...
def _hwnd_has_size(hwnd: int) -> bool:
    """Return True if the window has a non-zero client area."""
    rect = wintypes.RECT()
...
def strip_decorations(hwnd: int):
    """Remove title bar and borders so the VBox window acts as a child."""
    if not hwnd:
        return
...
class WindowFinderWorker(QThread):
    """
    Background thread that searches for the VirtualBox VM window.

    Two-phase approach:
      Phase 1 - Poll EnumWindows until the HWND appears (up to timeout_s)
      Phase 2 - Wait until the window has a non-zero client rect (rendered)

    This prevents embedding a blank VBox loading screen.
    """
    ...
    def run(self):
        ...
        if not hwnd:
            _log.warning(
                "WindowFinderWorker: window not found for '%s' in %ds",
        ...
        _log.info(
            "WindowFinderWorker: HWND 0x%08X found for '%s' - waiting for render",
            hwnd, self.vm_name,
        )

        # Phase 2: wait for client area to be non-zero
        ...
        if not _hwnd_has_size(hwnd):
            _log.warning(
                "WindowFinderWorker: HWND 0x%08X still zero-size after %ds - embedding anyway",
...
class VMViewport(QWidget):
    ...
    def _build_idle_panel(self) -> QWidget:
        ...
        title = StrongBodyLabel("Console Ready")
        title.setStyleSheet("font-size: 16px; color: rgba(255,255,255,0.7); background: transparent;")
    ...
    def _build_connecting_panel(self) -> QWidget:
        ...
        lay.setSpacing(18)
        icon = QLabel("[CONNECT]")
        icon.setStyleSheet("font-size: 24px; color: #60a5fa; background: transparent;")
        icon.setAlignment(Qt.AlignCenter)
        self._conn_title = StrongBodyLabel("Connecting...")
        self._conn_title.setStyleSheet(
            "font-size: 15px; color: #60a5fa; background: transparent;"
        )
        self._conn_title.setAlignment(Qt.AlignCenter)
        self._conn_sub = CaptionLabel(
            "Waiting for VirtualBox window...\n"
            "This usually takes 5-15 seconds after VM starts."
        )
        self._conn_sub.setAlignment(Qt.AlignCenter)
        self._conn_sub.setWordWrap(True)
        self._conn_sub.setStyleSheet("color: rgba(255,255,255,0.45); background: transparent;")
        self._spinner = _make_spinner(w)
        self._spinner.setFixedWidth(280)
        lay.addWidget(icon)
        lay.addWidget(self._conn_title)
        lay.addWidget(self._conn_sub)
    ...
    def _build_error_panel(self) -> QWidget:
        ...
        lay.setSpacing(16)
        icon = QLabel("[!]")
        icon.setStyleSheet("font-size: 48px; color: #ef4444; background: transparent;")
        icon.setAlignment(Qt.AlignCenter)
        self._err_title = StrongBodyLabel("Connection Failed")
        ...
        self._err_msg.setStyleSheet("color: rgba(255,255,255,0.45); background: transparent;")
        retry = PrimaryPushButton(FIF.SYNC, "Retry")
        retry.setFixedWidth(150)
        retry.clicked.connect(self.retry_requested)
        idle = PushButton(FIF.CLOSE, "Back to Idle")
        idle.setFixedWidth(150)
        idle.clicked.connect(lambda: self.disconnect())
        row = QHBoxLayout()
        row.setSpacing(12)
        row.addWidget(retry, alignment=Qt.AlignCenter)
        row.addWidget(idle,  alignment=Qt.AlignCenter)
        lay.addWidget(icon)
    ...
    def connect_vm(self, vm_name: str):
        ...
        Delays starting WindowFinderWorker by 3 seconds so VirtualBox has
        time to open its window after startvm returns.
        """
        self._vm_name = vm_name
        self.disconnect()   # reset state cleanly

        self._conn_title.setText(f"Connecting to '{vm_name}'...")
        self._conn_sub.setText(
            "Waiting for VirtualBox window...\n"
            "This usually takes 5-15 seconds after VM starts."
        )
        self._stack.setCurrentIndex(_IDX_CONNECTING)

        # 3s delay: let VBox open its GUI window before we search for it
        QTimer.singleShot(3000, lambda: self._start_finder(vm_name))

    def _start_finder(self, vm_name: str):
        """Start WindowFinderWorker after the initial delay."""
        if self._vm_name != vm_name:
            return   # user navigated away during the delay
        _log.info("VMViewport: starting WindowFinderWorker for '%s'", vm_name)
    ...
    def _do_embed(self, hwnd: int):
        ...
        try:
            ...
            _log.info("Embedded HWND 0x%08X via createWindowContainer", hwnd)
    ...
    def _validate_window_state(self):
        """Periodic check: if VBox window dies, reconnect automatically."""
        if not self._current_hwnd or self._stack.currentIndex() != _IDX_CONNECTED:
            return

        if not _u32.IsWindow(self._current_hwnd):
            _log.warning("VM window handle is dead - attempting reconnect")
            self._reconnect()
            return
        ...
        if w == 0 or h == 0:
            _log.warning("VM window size is 0 - attempting reconnect")
            self._reconnect()
    ...
    def _on_timeout(self):
        self._show_error(
            ...
            "* If the issue persists, restart the VM"
        )
